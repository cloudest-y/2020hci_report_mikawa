...
class NERLearner(object):
    ...
    def train(self, epoch, nbatches_train, train_generator, fine_tune=True):
        self.logger.info('\nEpoch: %d' % epoch)
        self.model.train()
    ...
    def test(self, nbatches_val, val_generator, fine_tune=False):
        ...
        self.logger.info("Val Loss : %.3f, Val Accuracy: %.3f%%,  Val Precision: %.3f%%,  Val Recall: %.3f%%, Val F1: %.3f%%" 
                         %(test_loss/(total_step+1), 100*acc, 100*p, 100*r, 100*f1))
        return 100*f1
